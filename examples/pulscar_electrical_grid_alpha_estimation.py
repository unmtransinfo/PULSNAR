...
def create_ml_dataset(data, pf, itr=0):
    ...
    """
    np.random.seed(itr)
    labels = data['stabf'].to_numpy()
    # print(Counter(labels))

    # find position of positive and unlabeled examples
    idx0, idx1 = np.where(labels == 'stable')[0], np.where(labels == 'unstable')[0]
...
#                   start of the code                    #
# ****************************************************** #
inpfile = "UCIdata/SCAR/electrical_grid_stability/Data_for_UCI_named.csv"
pfracs = [0.01, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60]  # positive fraction
n_iterations = 1
iter_alpha = {}

# get parameters from user for PULSNAR algorithm.
# update pulscar_electrical_grid_alpha.yaml if you want to override the default parameters
if len(sys.argv) < 2:
    user_param_file = 'testparams/pulscar_electrical_grid_alpha.yaml'
else:
...
# fetch electrical grid data and run PU
for pf in pfracs:
    for itr in range(n_iterations):
        ...
        pls = PULSNAR.PULSNARClassifier(scar=True, csrdata=False, classifier='xgboost',
                                        bin_method='scott', bw_method='hist', lowerbw=0.02, upperbw=0.5, optim='local',
...
print("results for electrical grid dataset are above")
