...
def read_input_file(ifile):
    ...
    df = df[df.iloc[:, 0].isin(['A', 'B', 'C', 'D', 'E', 'F'])]
    return df
...
def create_ml_dataset(data, frac, itr=0):
    ...
    # print("original count for all labels: ", Counter(labels))
...
#                   start of the code                    #
# ****************************************************** #
inpfile = "UCIdata/SNAR/letter_recognition/letter-recognition.data"
pfracs = [0.01, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60]  # positive fraction
n_iterations = 1
iter_alpha = {}

# get parameters from user for PULSNAR algorithm.
# update pulsnar_letter_alpha.yaml if you want to override the default parameters
if len(sys.argv) < 2:
    user_param_file = 'testparams/pulsnar_letter_alpha.yaml'
else:
...
print("results for letter recognition dataset are above")
