...
def create_ml_dataset(data, pf, itr=0):
    ...
    idx0, idx1 = np.asarray([i for i, l in enumerate(labels) if 't-' in l]), np.asarray([i for i, l in enumerate(labels) if 'c-' in l])
...
#                   start of the code                    #
# ****************************************************** #
inpfile = "UCIdata/SCAR/mouse_protein/Data.csv"
pfracs = [0.01, 0.05, 0.10, 0.20, 0.30, 0.40]  # positive fraction
n_iterations = 1
iter_alpha = {}

# get parameters from user for PULSNAR algorithm.
# update pulscar_mouse_protein_alpha.yaml if you want to override the default parameters
if len(sys.argv) < 2:
    user_param_file = 'testparams/pulscar_mouse_protein_alpha.yaml'
else:
...
# fetch mouse protein data and run PU
for pf in pfracs:
    for itr in range(n_iterations):
        ...
        pls = PULSNAR.PULSNARClassifier(scar=True, csrdata=False, classifier='xgboost',
                                        bin_method='scott', bw_method='hist', lowerbw=0.15, upperbw=0.5, optim='local',
...
print("results for mouse protein dataset are above")
