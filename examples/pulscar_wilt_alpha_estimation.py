...
def create_ml_dataset(data, pf, itr=0):
    ...
    idx0, idx1 = np.where(labels == 'w')[0], np.where(labels == 'n')[0]
...
#                   start of the code                    #
# ****************************************************** #
inpfile = "UCIdata/SCAR/wilt/training.csv"
pfracs = [0.01, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]  # positive fraction
n_iterations = 1
iter_alpha = {}

# get parameters from user for PULSNAR algorithm
# update pulscar_wilt_alpha.yaml if you want to override the default parameters
if len(sys.argv) < 2:
    user_param_file = 'testparams/pulscar_wilt_alpha.yaml'
else:
...
for pf in pfracs:
    for itr in range(n_iterations):
        ...
        pls = PULSNAR.PULSNARClassifier(scar=True, csrdata=False, classifier='xgboost',
                                        bin_method='fd', bw_method='hist', lowerbw=0.05, upperbw=0.5, optim='local',
...
print("results for wilt dataset are above")
